; $Id: //depot/idl/releases/IDL_80/idldir/lib/arrow.pro#1 $
...
;		vectors.
PRO ARROW_CLIP, x0, y0, x1, y1, HSIZE = hsize, COLOR = color, HTHICK = hthick, $
...
for i = 0L, n_elements(x0)-1 do begin		;Each vector
	...
	if keyword_set(solid) then begin	;Use polyfill?
	  ...
	  plots, [xp0, xp1+b*dx], [yp0, yp1+b*dy], /DEVICE, $
		COLOR = color, THICK = thick,noclip=0
	  polyfill, [xxp0, xxp1, xp1, xxp0], [yyp0, yyp1, yp1, yyp0], $
		/DEVICE, COLOR = color,noclip=0
	endif else begin
	  plots, [xp0, xp1], [yp0, yp1], /DEVICE, COLOR = color, THICK = thick,noclip=0
